...
Menu = offre commerciale composée de plusieurs produits sélectionnés selon des règles de composition. Dans ce projet, un menu comprend au minimum un burger, un accompagnement, une
boisson et une sauce. Le menu peut ensuite exister en variantes, par exemple normal ou maxi
...
RG-MENU-005 — Un menu peut contenir une sauce

Un menu peut contenir zéro ou une sauce, selon le choix du client.
...
- la boisson choisie ;
- la sauce choisie, si elle est sélectionnée ;
...
- un menu est composé d’un burger, d’un accompagnement, d’une boisson et d’une sauce ;
...
|-------|------|-------------|-------------|
| `numeroRetrait` | `string` | Oui | Numéro saisi par le client pour identifier sa commande au comptoir |
...
## 8.1 Architecture de la page `commande.html`
...
Ce fichier ne contient pas toute la logique métier. Il orchestre des modules spécialisés afin d’éviter un fichier central trop volumineux.
...
Ce dossier regroupe les styles réutilisables des blocs visuels présents dans `commande.html`.  
Il permet d’éviter d’entasser tout le style dans un seul fichier.

#### `css/pages/`

Ce dossier contient les styles spécifiques à la page `commande.html`, c’est-à-dire la mise en page globale de l’écran, ses zones principales et ses ajustements responsive.

#### `js/pages/`

Ce dossier contient les fichiers d’entrée des pages HTML.  
Dans le cas présent, `commande.js` initialise la page et coordonne les autres modules.

#### `js/services/`

Ce dossier regroupe les services techniques chargés de récupérer ou de persister les données.  
Il ne contient pas les règles métier de commande.

#### `js/state/`

Ce dossier regroupe les états temporaires nécessaires au fonctionnement de `commande.html`.  
Il sépare clairement l’état général de la page, l’état du panier et l’état du brouillon de sélection dans la modale.

#### `js/domain/`

Ce dossier contient les règles métier et les traitements purs de la page commande.  
On y place la normalisation des données, les règles de configuration d’un menu, les validations et les calculs de prix.

#### `js/ui/`

Ce dossier contient les fonctions de rendu de l’interface.  
Ces modules s’occupent d’afficher ou de mettre à jour les catégories, les produits, le panier, la modale et les messages.

#### `js/utils/`

Ce dossier contient les fonctions utilitaires transverses utilisées par plusieurs modules, sans porter de logique métier.
...
- l’état actif d’une catégorie ;
- les styles de survol, de focus et d’accessibilité.
...
- les images, titres et prix ;
- l’alignement des cartes ;
- les variations visuelles selon l’état des éléments.
...
- la liste des lignes du panier ;
- le bloc total ;
- les boutons associés au panier ;
- la lisibilité du résumé de commande.
...
- l’ouverture et la fermeture visuelle de la modale ;
- la structure interne des étapes ;
- les boutons d’action, de retour, d’annulation et de validation ;
- l’accessibilité visuelle de la fenêtre modale.

#### `js/pages/commande.js`
...
- récupérer l’état courant du panier déjà sauvegardé ;
- initialiser les états nécessaires à la page ;
- brancher les événements utilisateur ;
- déclencher les rendus de l’interface ;
- ouvrir, faire avancer, faire reculer ou fermer la modale de configuration ;
- demander la mise à jour du panier après validation d’une sélection ;
- préparer la transition vers la page suivante lorsque l’utilisateur poursuit son parcours.

`commande.js` ne doit pas contenir directement :

- les appels Ajax bas niveau ;
- la logique de calcul des prix ;
- les règles métier Best Of / Maxi Best Of ;
- la validation détaillée d’un menu ;
- les accès directs au `localStorage` ;
- le code de rendu détaillé du DOM.

Il agit comme un orchestrateur de page.

#### `js/services/ajax.js`

Ce fichier centralise les appels Ajax nécessaires au chargement des fichiers JSON.

Il gère :

- la récupération des données distantes ;
- les erreurs techniques de chargement ;
- la réponse brute retournée aux services consommateurs.

#### `js/services/catalog-service.js`

Ce fichier pilote la récupération du catalogue nécessaire à `commande.html`.

Il s’appuie sur `ajax.js` pour :

- charger `categories.json` ;
- charger `produits.json` ;
- transmettre ces données aux modules de normalisation ;
- fournir à `commande.js` un catalogue exploitable.

#### `js/services/storage-service.js`

Ce fichier centralise l’accès au stockage local utilisé par la page commande.

Il gère notamment :

- la lecture du panier existant ;
- l’écriture du panier mis à jour ;
- la lecture du type de commande ;
- la suppression des données temporaires si nécessaire.

#### `js/state/commande-state.js`

Ce fichier porte l’état global de la page `commande.html`.

Il mémorise par exemple :

- la catégorie actuellement affichée ;
- le produit ou menu actuellement sélectionné ;
- l’état de chargement général de la page ;
- l’ouverture ou la fermeture de la modale.

#### `js/state/cart-state.js`

Ce fichier représente l’état courant du panier dans la page commande.

Il contient notamment :

- les lignes du panier ;
- le total recalculé ;
- les informations utiles au rendu du panier ;
- la synchronisation entre état mémoire et stockage local.

#### `js/state/selection-modal-state.js`

Ce fichier représente le brouillon de sélection en cours dans la modale.

Il mémorise par exemple :

- le type de sélection en cours ;
- l’étape courante de la modale ;
- le type de menu choisi ;
- l’accompagnement choisi ;
- la boisson choisie ;
- la taille retenue ;
- les erreurs éventuelles de validation.

Ce brouillon est distinct du panier : tant que l’utilisateur n’a pas validé sa sélection, elle ne devient pas une ligne de panier.

#### `js/domain/normalize-catalog.js`

Ce fichier transforme les données JSON brutes dans un format exploitable par la page commande.

Il gère notamment :

- la normalisation de la structure des catégories ;
- la normalisation de la structure des produits ;
- la conversion des prix en centimes ;
- l’harmonisation des données utilisées par le rendu.

#### `js/domain/catalog-mapping.js`

Ce fichier porte les règles de correspondance entre les catégories affichées, les groupes de `produits.json` et les besoins de la maquette.

Il permet par exemple :

- de lier un onglet affiché à un groupe de produits ;
- de savoir quels groupes servent à alimenter la modale de configuration ;
- d’éviter de disperser ces correspondances dans plusieurs fichiers.

#### `js/domain/menu-rules.js`

Ce fichier contient les règles métier propres à la configuration d’un menu.

Il gère notamment :

- les choix Best Of / Maxi Best Of ;
- les conséquences d’un choix de taille ;
- les groupes de produits autorisés pour l’accompagnement, la boisson et la sauce ;
- les règles de composition d’un menu complet.

#### `js/domain/menu-validator.js`

Ce fichier valide une configuration de menu en cours.

Il vérifie notamment :

- que les choix obligatoires sont bien présents ;
- que les tailles choisies sont cohérentes ;
- qu’un menu peut être transformé en ligne de panier valide.

#### `js/domain/product-validator.js`

Ce fichier valide une sélection simple de produit lorsqu’une configuration complémentaire est nécessaire.

Il sert par exemple à vérifier qu’un produit configuré dans une modale est complet avant ajout au panier.

#### `js/domain/price-calculator.js`

Ce fichier calcule les prix nécessaires à la page commande.

Il gère notamment :

- le prix d’une ligne ;
- le supplément éventuel lié au type de menu ou à la taille ;
- le total du panier ;
- le travail exclusif en centimes.

#### `js/domain/cart-validator.js`

Ce fichier valide l’état global du panier dans la page commande.

Il permet notamment :

- de vérifier qu’une ligne est exploitable ;
- de garantir la cohérence du panier avant poursuite du parcours ;
- d’autoriser ou non l’action menant à la page suivante.

#### `js/ui/render-categories.js`

Ce fichier affiche la zone des catégories et met à jour leur état visuel.

Il gère notamment :

- l’affichage des catégories disponibles ;
- la mise en évidence de la catégorie active ;
- la mise à jour visuelle après changement de sélection.

#### `js/ui/render-products.js`

Ce fichier affiche les produits correspondant à la catégorie ou au menu sélectionné.

Il gère notamment :

- la génération des cartes produit ;
- l’affichage des titres, images et prix ;
- l’actualisation de la grille de produits selon l’état courant.

#### `js/ui/render-cart.js`

Ce fichier affiche le panier latéral de `commande.html`.

Il gère notamment :

- l’affichage des lignes de panier ;
- le résumé des sélections ;
- l’affichage du total ;
- la mise à jour du panier après chaque modification.

#### `js/ui/render-selection-modal.js`

Ce fichier gère l’affichage de la modale de configuration de sélection.

Il s’occupe notamment :

- d’afficher l’étape courante ;
- d’afficher les options possibles ;
- d’actualiser le contenu de la modale à chaque changement d’étape ;
- d’afficher les boutons d’action liés à la modale.

#### `js/ui/render-messages.js`

Ce fichier centralise l’affichage des messages visibles sur `commande.html`.

Il permet notamment :

- d’afficher un message d’erreur ;
- d’afficher un message de blocage ;
- d’afficher un retour utilisateur après une action.

#### `js/utils/dom.js`

Ce fichier contient des helpers de manipulation du DOM utilisés par plusieurs modules de rendu ou de contrôle.

#### `js/utils/format-price.js`

Ce fichier contient les fonctions de formatage des prix pour l’affichage, à partir des centimes calculés côté métier.
...
3. `normalize-catalog.js` et `catalog-mapping.js` préparent les données exploitables ;
4. `storage-service.js` recharge le panier existant ;
5. `commande-state.js`, `cart-state.js` et `selection-modal-state.js` portent les états temporaires de la page ;
6. `render-categories.js`, `render-products.js`, `render-cart.js` et `render-selection-modal.js` affichent l’interface ;
7. `menu-rules.js`, `menu-validator.js`, `product-validator.js`, `price-calculator.js` et `cart-validator.js` garantissent la cohérence des traitements ;
8. `commande.js` orchestre l’ensemble sans contenir lui-même toute la logique métier.

## 8.2 Architecture des autres pages

En complément de `commande.html`, l’application repose sur trois autres pages HTML plus simples :

- `index.html` ;
- `retrait.html` ;
- `confirmation.html`.

Ces pages réutilisent la même logique générale de découpage :

- un fichier HTML pour la structure de la page ;
- un fichier JS d’entrée pour piloter la page ;
- un fichier CSS de page pour la mise en page spécifique ;
- les modules partagés déjà définis pour les besoins communs.
...
- `retrait.html`
- `js/pages/retrait.js`
- `css/pages/retrait.css`

Rôle de `retrait.js` :

- afficher le formulaire ou le clavier de saisie ;
- récupérer le panier et le type de commande déjà validés dans le parcours ;
- enregistrer le numéro saisi ;
- vérifier que la saisie est exploitable ;
- déclencher la validation finale et la poursuite du parcours.

Cette page peut réutiliser des modules partagés tels que :

- `storage-service.js` ;
- `cart-validator.js` ;
- les modules de construction et d’envoi de la commande finale.
...
- `confirmation.html`
- `js/pages/confirmation.js`
- `css/pages/confirmation.css`

Rôle de `confirmation.js` :

- afficher le message final de confirmation ;
- confirmer à l’utilisateur que la commande a bien été prise en compte ;
- remettre la borne dans un état propre ;
- supprimer les données temporaires de commande ;
- permettre le lancement d’une nouvelle commande.
...
- `retrait.html` pour la saisie du numéro final ;
- `confirmation.html` pour la fin du parcours ;
- un fichier JS d’entrée par page ;
- des modules partagés pour la logique métier, les services techniques, les états temporaires, le rendu et les utilitaires.
